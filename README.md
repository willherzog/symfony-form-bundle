...
* `module/functions.mjs` – Exports the `activateToggleSwitch()`, `makeStylableCheckboxWidget()`, `makeStylableRadioWidget()`, `enableCollapsibleFieldsets()`, `moveCursorAfterLastCharacter()`, `changeSelectOptionsEnablement()`, `createRemoveButtonElement()` and `setupSubFormRemoveAction()` functions (the latter two are intended for use with entries of `CollectionType` field groups)
