			...
			->children()
				...
				->arrayNode('form')
					...
					->children()
						...
						->end()
						->booleanNode('use_separate_value_label')
							->defaultFalse()
							->info('Whether the modal select field types (ModalSelectType, ModalImageSelectType, ModalEnumSelectType) should have a separate "value label" element (instead of using the button text itself for this purpose). Note: By default, these fields have the options "button_text" and "default_button_label", but if this is enabled they will instead have the options "button_label", "value_label" and "unset_value_label".')
						->end()
					->end()
				->end()
			->end()
	...
	public function loadExtension(array $config, ContainerConfigurator $container, ContainerBuilder $builder): void
	{
		...
		$container->services()
			...
			->set('whform.type.button_with_label', ModalSelectType::class)
				->args([$config['form']['default_optional'], $config['form']['use_separate_value_label']])
				->call('setPropertyAccessor', [service('form.property_accessor')->ignoreOnInvalid()])
